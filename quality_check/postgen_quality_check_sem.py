    ...
    if len(row[column_map['img_id']]):
        ...
        # print("Calculating metrics for ", row[column_map['img_id']])
            ...
            if (dataset == DS) and (model == MODEL) and (language_model == LANG):
                ...
                orig_img_path = os.path.join(PATH_TO_ORIG_DATA_PARENT, dataset, row[column_map['img_id']]+DS_EXTENSION)
                ...
                perturbed_img_path = os.path.join(PATH_TO_PERTURB_DATA_PARENT, language_model, dataset, model_map[model] , row[column_map['perturbed_img_id']])
