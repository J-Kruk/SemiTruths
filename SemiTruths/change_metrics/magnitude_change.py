...
class Change:
    ...
    def plot_cc(self,labeled_image):
        ...
        colors = plt.cm.get_cmap('tab20', num_unique_values - 1)  # Choose a colormap and get unique colors, excluding 0
    ...
    def calc_metrics(self,real_img_path,fake_img_path,og_caption ='',edit_caption =''):
        ...
        if(has_nan_fake) or (has_nan_real):
            ...
            # ratio_gray, labeled_image_gray, cc_clusters_gray, cluster_dist_gray, total_pixels_gray,largest_component_size_gray, _, _ = self.calc_cc_ratio_new(binary_gray)
            # save_path = os.path.join('/srv/share4/apal72/half-truths/data/Half_Truths_Dataset/generated_images/prompt-prompt_subset_qual_1/masks',fake_img_path.split('/')[-4],fake_img_path.split('/')[-3],fake_img_path.split('/')[-2])
            # if(not(os.path.exists(save_path))):
            #     os.makedirs(save_path)
            # cv2.imwrite(os.path.join(save_path, fake_img_path.split('/')[-1]),save_img)
            # if(self.save_dir):
            #     save_path = os.path.join(self.save_dir, 'images_new_31',fake_img_path.split('/')[-3],fake_img_path.split('/')[-2])
            #     if(not(os.path.exists(save_path))):
            #         os.makedirs(save_path)
            #     self.save_img(img_real_rgb, img_fake_rgb, img_fake_gray,mse_img_rgb, mse_img_gray,labeled_image_rgb, labeled_image_gray, ratio_gray, ratio_rgb, mse_rgb, mse_gray, cc_clusters_rgb, cc_clusters_gray, cluster_dist_rgb, cluster_dist_gray, total_pixels_rgb, total_pixels_gray, dilated_image,os.path.join(save_path, fake_img_path.split('/')[-1]))
