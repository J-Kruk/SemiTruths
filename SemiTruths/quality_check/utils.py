...
def calculate_caption_similarity(caption1, caption2):
    """
    Similarity between the original and perturbed captions.
    Image and caption embeddings are generated by CLIP.

    Inputs:
    ----------------
    caption1 : str
        Original caption or mask entity label.
    caption2 : str
        Perturbed caption or mask entity label.
...
def prune_length(orig_cap, edit_cap):
    """
    Returns a boolean statement signalig is the
    edited caption is substantially larger than the
    original.

    Inputs:
    ----------------
    orig_cap : str
        Original image caption or mask label.
    edit_cap : str
        Perturbed image caption or mask label.

    Returns:
    ----------------
    output : boolean
        True if edit caption is approximately same
        length as original caption.
    """
...
def calculate_image_caption_similarity(image, caption):
    """
    Similarity between the perturbed caption and augmented image.
    Image and caption embeddings are generated by CLIP.
...
def calculate_directional_similarity(image1, caption1, image2, caption2):
    """
    Computing directional similariy between the change in
    caption1 to caption2, and image1 and image2. This communicates
    if the image and captions were changes in the same semantic
    "direction". Image and caption embeddings are generated by CLIP.
    ...
    image2 : PIL.Image
        Augmented image.
    caption2 : str
        Augmented caption or mask entity label.
